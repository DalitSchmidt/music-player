...
const AlbumPlayer = {
    ...
    playSong: function( e ) {
        ...
        let el = $( e.target )
        // מחיקת ה- class בשם playing מהתגית li המכילה את ה- class בשם playing ושמצויה תחת div המכיל מזהה ייחודי בשם current-playlist
        ...
        // פנייה ל- attribute מסוג src של תגית iframe שמצויה תחת מזהה ייחודי בשם album-cover שבו נחליף בקישור את המזהה הייחודי של כל קליפ
        ...
        // החלפת הטקסט המצוי ב- span המכיל מזהה ייחודי בשם song-name ומצוי תחת span המכיל מזהה ייחודי בשם now-playing-song בטקסט המצוי במשתנה song_name
    ...
    switchDetails: function( e ) {
        // המשתנה el מאפשר לנו לבצע פעולות על האלמנט שהפעיל את ה- event
        let el = $( e.target )
        // מחיקת ה- class בשם active מה- div המכיל מזהה ייחודי בשם menu
    ...
    // הפונקציה bindEvents מכיל את כל ה- eventים הקורים באובייקט AlbumPlayer
    bindEvents: function() {
        // כאשר לוחצים על הנתונים המצויים בתגית li שמצויה תחת התגית ol המכילה מזהה ייחודי בשם current-playlist, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם playSong יתייחס לאלמנט עצמו (במקרה זה לאלמנט li שמצוי תחת התגית ol המכילה מזהה ייחודי בשם current-playlist) נשתמש ב- proxy, כדי שההקשר של this בתוך הפונקציה playSong יתייחס בכל מקרה לאובייקט AlbumPlayer
