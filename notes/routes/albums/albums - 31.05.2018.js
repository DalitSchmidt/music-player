...
router.get('/suggestions/artist/:term', function ( req, res ) {
    ...
    // בבקשה אנחנו מחפשים את כל המידע הקיים במשתנה AlbumModel (המכיל את המודל של Album המצוי תחת המשתנה models) ב- attributes בשם album_artist שמצוי בקבוצה המתאימה, איפה שמצויים הנתונים של השדה album_artist בהתאם לנתונים שמצויים במשתנה term (המכיל את הנתונים שהוזנו ושלגביהם מתקבלות ההצעות האפשריות לתוצאות החיפוש), ואז מאחר ואנו משתמשים ב- sequelize שהוא מודול המבוסס על promise, נפעיל promise על התוצאות, כך שאם אין תווים במשתנה results והוא לא מכיל תוצאות, אז נשלח הודעת סטטוס עם הקוד 204 (No Content), האומרת שהשרת עיבד בהצלחה את הבקשה, אך אינו מחזיר תוכן, אחרת, כלומר יש תווים במשתנה results והוא מכיל תוצאות, נבצע מיפוי לתוצאות שנקבל של האמן ונשלח תשובת json עם התוצאות
    AlbumModel.findAll({
...
router.post('/', function ( req, res ) {
    ...
    // נבדוק אם לפרופרטי genres המצוי במשתנה album יש ערכים, למעשה אנו בודקים את האורך של המערך של הז'אנרים ואם הוא מכיל 0 ערכים, כלומר שהוא ריק מערכים
    ...
    AlbumModel.create( album )
        ...
        // ואז מאחר ואנו משתמשים ב- sequelize שהוא מודול המבוסס על promise נפעיל promise שפונקציית ה- callback שלו מבצעת מספר פעולות
        .then(genre_results => {
            // המשתנה new_genres_ids מכיל את המשתנה genre_results שמפעיל את הפונקציה map היוצרת מערך חדש עם התוצאות של הפונקציה הקוראת בכל אלמנט במערך, כאשר במקרה שלנו היא משייכת את ה- genre_id
            ...
            // הפונקציה מחזירה את המשתנה AlbumToGenresModel שמפעיל את הפונקציה bulkCreate היוצרת ומוסיפה את כל האירועים המצויים במערך של album_genres_ids, כך שלמעשה היא מוסיפה את המזהה הייחודי של הז'אנרים שלגביהם מבוצע עדכון של הנתונים
            return AlbumToGenresModel.bulkCreate( album_genres_ids )
        })
        // ואז מאחר ואנו משתמשים ב- sequelize שהוא מודול המבוסס על promise נפעיל promise שפונקציית ה- callback שלו מקבלת את המשתנה result ומבצעת מספר פעולות
        .then(result => {
        ...
        .catch(err => {
            ...
            if ( album_id ) {
                ...
                AlbumsController.deleteAlbum( album_id ).spread(affected_rows => {
                    ...
                    // אם התוצאה היא אחרת, כלומר 1, כי זה המספר המקסימלי של השורות שיכולות להיות מושפעות מאחר ואנו מבקשים למחוק נתונים מאלבום ספציפי לפי המספר id שלו ויכול להיות רק אלבום אחד שמכיל את אותו מספר id, אז נבצע מספר פעולות
                    else {
...
router.put('/:album_id', function ( req, res ) {
    ...
    // במידה ויש שגיאות המונעות מאיתנו לעדכן את האלבום נפעיל promise התופס את השגיאות ושפונקציית ה- callback שלו (המסומנת כפונקציית חץ) מקבלת את המשתנה err
    }).catch(err => {
        // המשתנה errors מכיל את השגיאה שקיבלנו
        let errors = err.errors[0]

        // במידה וקיבלנו הודעת שגיאה המכילה את הסטטוס קוד 422 (Unprocessable Entity) האומרת שהשרת מבין את סוג התוכן של יישות הבקשה, אך לא הצליח לעבד את ההוראות הכלולות בו מאחר ולא ניתן לעדכן את האלבום ונשלח json המפרט את הודעת השגיאה שקיבלנו
        res.status(422).json({
