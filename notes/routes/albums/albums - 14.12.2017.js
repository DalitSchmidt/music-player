...
router.post('/', function ( req, res ) {
    ...
    AlbumModel.create( album ).then(result => {
        ...
        // המשתנה songs מפעיל את הפונקציה map היוצרת מערך חדש עם התוצאות של הפונקציה הקוראת בכל אלמנט במערך, כאשר במקרה שלנ היא משייכת כל שיר לאלבום שלו לפי המספר id של האלבום ומחזירה את השיר
        ...
        // המשתנה SongModel מפעיל את הפונקציה bulkCreate היוצרת ומוסיפה את כל האירועים המצויים במערך של songs, כך שלמעשה היא מוסיפה את כל השירים לתוך האלבום שיווצר, ומאחר ואנו משתמשים ב- sequelize שהוא מודול המבוסס על promise, נפעיל promise על יצירת האלבום, וכך למעשה אנו יוצרים את השירים במסד הנתונים ומקשרים אותם לאלבום הרלוונטי
