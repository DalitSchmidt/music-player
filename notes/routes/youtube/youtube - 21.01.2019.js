    ...
    if ( youtube_id.length < 8 || youtube_id.length > 14 )
        ...
        request('https://www.youtube.com/oembed?format=json&url=http://www.youtube.com/watch?v=' + youtube_id, ( error, response, body ) => {
            ...
            if ( response.statusCode === 200 ) {
                ...
                // המשתנה request שולח בקשה לכתובת ה- URL 'https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${youtube_id}&key=${process.env.YOUTUBE_API_KEY}' שמביאה את הנתונים של השיר בפורמט json בהתאם למזהה הייחודי של youtube ולמפתח שמוגדר במשתנה process.env.YOUTUBE_API_KEY, ומפעיל פונקציית callback (המסומנת כפונקציית חץ) המקבלת את המשתנים error, response ו- body
