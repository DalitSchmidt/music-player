...
const Router = {
    ...
    // באמצעות הפונקציה setupRoutes המקבלת את המשתנה routes מתאפשר "להכין" את הנתיבים לשימוש
    setupRoutes: function( routes ) {
        // הפרופרטי routes מכיל את המשתנה routes, כך שלמעשה הוא מכיל בתוך האובייקט את הנתיבים שיבוצעו בהם שימוש
        this.routes = routes
        // הפונקציה מחזירה את הנתיבים שיבוצעו בהם שימוש
        return this
    },
    ...
    init: function() {
        ...
        if ( page ) {
            ...
            this.setPage().then(() => {
                ...
                this.bindEvents()
                // המשתנה callback מכיל את הנתונים המצויים בפרופרטי routes המכיל אובייקט שבתוכו מצויים למעשה הנתיבים שיבוצעו בהם שימוש, בהתאם לנתונים המצויים במשתנה page המפעיל את הפונקציה checkURLHash שבאמצעותה אנו בודקים אם כתובת ה- URL מכילה את הסימן # או לא
                let callback = this.routes[ page ]
                // הפעלה של פונקציית ה- callback
                callback()
            })
        } else
            // אחרת, כלומר שהמשתנה page לא הפעיל את הפונקציה checkURLHash שבאמצעותה אנו בודקים אם כתובת ה- URL מכילה את הסימן # או לא, נפעיל את הפונקציה bindEvents המכילה את כל ה- eventים הקורים באובייקט Router, כך שלמעשה בכל מקרה אנו מפעילים את הפונקציה bindEvents המכילה את כל ה- eventים הקורים באובייקט Router
            this.bindEvents()
    }
}
