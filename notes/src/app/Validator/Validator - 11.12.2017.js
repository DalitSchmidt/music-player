...
// ייבוא היכולות של האובייקט Utils על-מנת שהאובייקט Validator יוכל להשתמש בהן
...
const Validator = {
    ...
    validateField: function ( input ) {
        ...
        if ( !this.regexes[ input_name ].test( input_value ) ) {
            ...
            // המשתנה html מפעיל את הפונקציה validateInput המקבלת את המשתנה error_message המכיל את הודעת השגיאה המפורטת בהתאם לשדה שמצוי במשתנה input_name שמכיל את כל ה- inputים שיש להם attribute בשם name ומצויה תחת האובייקט AlbumFormTemplates ושבאמצעותה אנו יוצרים תבנית html המציגה ב- DOM את הודעת השגיאה
    ...
    validateDuplications: function ( arr_to_check, key_to_check, value_to_check, error_key, error_container ) {
        ...
        // המשתנה has_duplictions מפעיל את הפונקציה isInArrayOfObjects המקבלת את המשתנים arr_to_check, key_to_check ו- value_to_check המצויה תחת האובייקט Utils ושבאמצעותה מתאפשר לבדוק אם המערך הוא מערך של אובייקטים
        ...
        if ( has_duplications ) {
            // המשתנה html מפעיל את הפונקציה validateInput המקבלת את הפרופטי error_message במקום של המשתנה error_key במערך ומצויה תחת האובייקט AlbumFormTemplates ושבאמצעותה אנו יוצרים תבנית html המציגה ב- DOM את הודעת השגיאה, כך שלמעשה המשתנה html מאפשר להציג ב- DOM את הודעת השגיאה הרלוונטית
            let html = AlbumFormTemplates.validateInput( this.error_messages[ error_key ] )
