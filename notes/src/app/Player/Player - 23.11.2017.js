...
const Player = {
    ...
    bindEvents: function() {
        // המשתנה that מכיל את ההקשר של האובייקט
        let that = this
        ...
        // בעת ביצוע לחיצה על הנתונים המצויים בתגית li שמצויה תחת התגית ol המכילה מזהה ייחודי בשם player-playlist, פונקציית ה- callback מקבלת את המשתנה e המסמל event
        $('#player-playlist li').on('click', function( e ){
            // המשתנה el  מאפשר לנו לבצע פעולות על האלמנט שהפעיל את ה- event
            let el = $( e.target )
            // המשתנה that מפעיל את הפונקציה playSong שמקבלת את המשתנה el שמאפשר לנו לבצע פעולות על האלמנט שהפעיל את ה- event
            that.playSong(el)
        })
