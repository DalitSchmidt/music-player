...
const Player = {
    ...
    playPreviousSong: function () {
        // המשתנה previous_song מכיל את האלמנט li הראשון שלפני אלמנט li שיש לו class בשם playing, כך שלמעשה הוא מכיל את השיר המצוי ברשימת השירים לפני השיר שמתנגן
        let previous_song = $('li.playing').before()

        // אם אין תווים במשתנה previous_song, כלומר שאורך התווים במשתנה previous_song הוא 0
    ...
    // באמצעות הפונקציה changeVolume מתאפשר לשנות את עוצמת השמע של הסרטון שמתנגן בנגן
    changeVolume: function () {
        // הבאת הערך המצוי באלמנט input שה- attribute מסוג name שלו הוא volume
        this.player.volume = $('.audio-player input[name="volume"]').val()
        // שינוי ה- icon של ה- volume בהתאם למיקום של הסמן באלמנט input שה- attribute שלו הוא volume
