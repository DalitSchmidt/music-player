...
import $ from 'jquery'
// ייבוא היכולות של ה- class DataService על-מנת שהאובייקט SearchBar יוכל להשתמש בהן
import DataService from './DataService'
// ייבוא היכולות של ה- class Templates על-מנת שהאובייקט SearchBar יוכל להשתמש בהן
...
const SearchBar = {
    ...
    searchAlbum: function () {
        ...
        // הצגת הודעה עם הנתונים שהוזנו במשתנה term
        alert("/search/" + term)
    },
    ...
    bindEvents: function() {
        // כאשר מתבצעת לחיצה על האלמנט button שיש לו מזהה ייחודי בשם search, ומאחר ואנו רוציםי שההקשר של this בתוך פונקציית ה- callback בשם searchAlbum יתייחס לאלמנט עצמו (במקרה זה כפתור החיפוש), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה searchAlbum יתייחס בכל מקרה לאובייקט searchAlbum
