...
const SearchBar = {
    ...
    clearResults: function() {
        // הסתרת הנתונים המצויים באלמנט div שיש לו מזהה ייחודי בשם results-list לאחר 3 מאיות השנייה כשפונקציית ה- callback שלה מבצעת מספר פעולות שונות
        $('#results-list').hide(300, function() {
            ...
            $('#search-term').val('')
            // הסרת ה- class בשם has-suggestions מהאלמנט שיש לו class בשם pull-right
    ...
    getSuggestions: function() {
        ...
        SearchAPIService.suggestions( term ).then(( response, textStatus, xhr ) => {
            ...
            // הכנסת המשתנה html לתוך אלמנט div שיש לו מזהה ייחודי בשם results-list, ובכך אנו למעשה מציגים ב- DOM את ההצעות האפשריות לתוצאות החיפוש ונוסיף לאלמנט הקרוב ביותר שיש לו class בשם pull-right את ה- class בשם has-suggestions
    ...
    bindEvents: function() {
        ...
        // כאשר תבוצע יציאה של המשתמש עם העכבר מאלמנט המצוי בתוך האלמנט header שיש לו class בשם pull-right ושיש לו class בשם has-suggestions נפעיל את הפונקציה clearResults המאפשרת לנקות את תיבת ההצעות האפשריות לתוצאות החיפוש, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם clearResults יתייחס לאלמנט עצמו (במקרה זה לאלמנט header שיש לו class בשם pull-right ושיש לו class בשם has-suggestions) נשתמש ב- proxy, כדי שההקשר של this בתוך הפונקציה clearResults יתייחס בכל מקרה לאובייקט SearchBar
        $('header').on('mouseleave', '.pull-right.has-suggestions', $.proxy( this.clearResults, this ))
        // בעת ביצוע לחיצה על האלמנט li המצוי בתוך אלמנט שיש לו class בשם pull-right ובשם has-suggestions), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה clearResults יתייחס בכל מקרה לאובייקט SearchBar
