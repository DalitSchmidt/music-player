...
import $ from 'jquery'
// ייבוא היכולות של האובייקט DataService על-מנת שהאובייקט Search יוכל להשתמש בהן
...
const Search = {
    ...
    searchAlbum: function() {
        ...
        // הפעלה של הפונקציה searchAlbums המקבלת את המשתנה term (המכיל את הטקסט שהוזן בתיבת החיפוש) ושמצוי תחת האובייקט DataService המאפשרת לבצע חיפוש של נתונים באמצעות ביצוע בקשת get לנתיב 'http://localhost:3000/api/albums/search' + term, כאשר לאחר הפעלה הפונקציה נפעיל promise על התוצאות המתקבלות באמצעות הפעלה של הפונקציה displayResults המקבלת את המשתנים term ו- results שבאמצעותה מתאפשר להציג ב- DOM את תוצאות החיפוש
        DataService.searchAlbums( term ).then(results => this.displayResults( term, results ))
    },
