...
const AlbumsBoard = {
    ...
    appendAlbums: function( albums ) {
        ...
        // ביצוע השהייה של שנייה לפני הכנסת המשתנה html המכיל תבנית html של האלבום לתוך אלמנט div שיש לו מזהה ייחודי בשם album-list, ובכך מתאפשר להציג ב- DOM את כל האלבומים הקיימים לאחר שה- loader הוסר מה- DOM
        }).delay( 1000, function() {
    ...
    applyAlbums: function() {
        // הפעלה של הפונקציה getAllAlbums המאפשרת לקבל את האלבומים על-ידי ביצוע בקשת get לנתיב 'http://localhost:3000/api/albums', ולאחר מכן נפעיל promise שפונקציית ה- callback שלו מקבלת את המשתנה albums
        this.getAllAlbums().then(albums => {
            // נבדוק אם המשתנה albums הוא undefined, אם כן, אז נפעיל את הפונקציה noAlbums שבאמצעותה מתאפשר להציג הודעה מתאימה ב- DOM כאשר אלבומים שמורים במסד הנתונים
