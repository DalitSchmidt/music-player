...
const EditAlbum = {
    ...
    // באמצעות הפונקציה removeSongMessage המקבלת את המשתנה e (המסמל event) מתאפשר להציג הודעת מחיקה כאשר אנו מעוניינים למחוק שיר
    removeSongMessage: function ( e ) {
        ...
        // AlbumAPIService.getAlbumById( song_id ).then(album => {
            // המשתנה html מפעיל את הפונקציה removeSongDialog המקבלת את המשתנה song_id המצויה תחת האובייקט EditAlbumTemplates ושבאמצעותה אנו יוצרים תבנית html המציגה ב- DOM הודעה מתאימה בדבר מחיקת השיר הרלוונטי
            ...
            // הוספת ה- class בשם in עם תכונות ה- css בשם display שהערך שלה הוא block ו- padding-right שהערך שלה הוא 0 לאלמנט div שיש לו מזהה ייחודי בשם modal
            $('#modal').addClass('in').css({'display': 'block', 'padding-right': '0'})
        // })
    },

    // באמצעות הפונקציה confirmRemoveSong המקבלת את המשתנה e (המסמל event) מתאפשר לבצע פעולות עם אישור מחיקת השיר
    confirmRemoveSong: function ( e ) {
        ...
        // AlbumAPIService.deleteAlbum( song_id ).then(album => {
            ...
            $('.modal-dialog').fadeOut('slow', () => {
                // המשתנה html מפעיל את הפונקציה removeSongSuccessDialog המקבלת את המשתנה song_id המכיל את האלמנט הקרוב ביותר עם המספר id של השיר, שמצויה תחת האובייקט EditAlbumTemplates ושבאמצעותה אנו יוצרים תבנית html המציגה ב- DOM הודעה המאשרת שהשיר נמחק
    ...
    // באמצעות הפונקציה cancelRemove מתאפשר לחזור לעמוד שבו היינו בעת לחיצה על האלמנט שיש לו את ה- class בשם cancel, ולמעשה לבטל את פעולת המחיקה
    ...
    bindEvents: function () {
        // בעת ביצוע לחיצה על האלמנט שיש לו class בשם remove-icon שמצוי בתוך האלמנט div שיש לו מזהה ייחודי בשם add-album-playlist-form, נפעיל את הפונקציה removeSongMessage המאפשרת להציג הודעת מחיקה כאשר אנו מעוניינים למחוק שיר, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם removeSongMessage יתייחס לאלמנט עצמו (במקרה זה לאלמנט שיש לו class בשם remove-icon), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה removeSongMessage יתייחס בכל מקרה לאובייקט EditAlbum
        ...
        // בעת ביצוע לחיצה על אלמנט button שיש לו מזהה ייחודי בשם edit-album-approve-delete שמצוי בתוך האלמנט שיש לו class בשם modal-dialog נפעיל את הפונקציה confirmRemoveSong שבאמצעותה מתאפשר לבצע פעולות עם אישור מחיקת השיר, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם confirmRemoveSong יתייחס לאלמנט עצמו (במקרה זה לאלמנט button שיש לו מזהה ייחודי בשם edit-album-approve-delete), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה confirmRemoveSong יתייחס בכל מקרה לאובייקט EditAlbum
        $('.modal-dialog').on('click', '#edit-album-approve-remove', $.proxy( this.confirmRemoveSong, this ))
        // בעת ביצוע לחיצה על אלמנט button שיש לו מזהה ייחודי בשם edit-album-approve-delete שמצוי בתוך האלמנט שיש לו class בשם modal-dialog נפעיל את הפונקציה confirmRemoveSong שבאמצעותה מתאפשר לבצע פעולות עם אישור מחיקת השיר, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם confirmRemoveSong יתייחס לאלמנט עצמו (במקרה זה לאלמנט button שיש לו מזהה ייחודי בשם edit-album-approve-delete), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה confirmRemoveSong יתייחס בכל מקרה לאובייקט EditAlbum
        $('.modal-dialog').on('click', '#edit-album-approve-remove', $.proxy( this.confirmRemoveSong, this ))
        // בעת ביצוע לחיצה על האלמנט שיש לו class בשם cancel שמצוי בתוך האלמנט שיש לו class בשם modal-dialog נפעיל את הפונקציה cancelRemove שבאמצעותה מתאפשר לחזור לעמוד שבו היינו, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם cancelRemove יתייחס לאלמנט עצמו (במקרה זה לאלמנט שיש לו class בשם cancel), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה cancelRemove יתייחס בכל מקרה לאובייקט EditAlbum
