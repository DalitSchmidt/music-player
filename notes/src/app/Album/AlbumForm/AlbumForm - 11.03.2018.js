...
const AlbumForm = {
    ...
    // באמצעות הפונקציה emptyValues המקבלת את המשתנה item מתאפשר לרוקן מערכים במידת הצורך את האלמנט span כאשר אין נתונים באלמנט input שה- attribute מסוג name שלו הוא song_youtube, כלומר שאין נתונים באלמנט input של המזהה הייחודי של YouTube
    emptyValues: function ( item ) {
        // המשתנה song מכיל באמצעות שימוש ב- jQuery את ה- item שמצוי במערך של song-item
        let song = $( item )
        // המשתנה song_youtube מכיל את הערך המצוי בתיבת ה- input של המזהה הייחודי של YouTube, באמצעות מציאת הנתונים המצויים במשתנה song (המכיל את ה- item שמצוי במערך של song-item) את ה- input שיש לו attribute מסוג name בשם song_youtube
        let song_youtube = song.find('input[name=song_youtube]')

        // נבדוק אם אין ערכים במשתנה song_youtube המכיל את הערך המצוי בתיבת ה- input של המזהה הייחודי של YouTube, באמצעות מציאת הנתונים המצויים במשתנה song (המכיל את ה- item שמצוי במערך של song-item) את ה- input שיש לו attribute מסוג name בשם song_youtube
        if ( song_youtube.val('') ) {
            // אז נכניס לאלמנט span שמצוי בתוך האלמנט שיש לו class בשם form-group טקסט ריק
            $('.form-group span').text('')
        }
    },
    ...
    bindEvents: function () {
        ...
        // כאשר מבוצעת יציאה מהאלמנט על-ידי המשתמש מאלמנט input שיש לו את ה- attribute מסוג name בשם song_youtube שמצוי בתוך האלמנט div שיש לו מזהה ייחודי בשם add-album-playlist-form, ומאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם emptyValues יתייחס לאלמנט עצמו (במקרה זה לאלמנט input שיש לו את ה- attribute מסוג name בשם song_youtube), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה emptyValues יתייחס בכל מקרה לאובייקט AlbumForm
        ...
        // כאשר מתבצעת יציאה של המשתמש מהאלמנט input שיש לו class בשם error ו/או מהאלמנט span שיש לו class בשם error ו/או מהאלמנט שיש לו class בשם song-item שיש לו class בשם error ושהוא מצוי בתוך אלמנט div שיש לו מזהה ייחודי בשם add-album-playlist-form, מאחר ואנו רוצים שההקשר של this בתוך פונקציית ה- callback בשם validateField יתייחס לאלמנט עצמו (במקרה זה לאלמנט input שיש לו class בשם error ו/או מהאלמנט span שיש לו class בשם error ו/או מהאלמנט שיש לו class בשם song-item שיש לו class בשם error), לכן נשתמש ב- proxy כדי שההקשר של this בתוך הפונקציה validateField יתייחס בכל מקרה לאובייקט AlbumForm
