...
const AlbumForm = {
    ...
    validateAlbum: function () {
        ...
        if ( !album ) {
            // false נפעיל את הפונקציה scrollTop המבצעת גלילה של העמוד חזרה למעלה עד לתחילת האלמנט div שיש לו מזהה ייחודי בשם main-container ונחזיר את הערך הבוליאני
    ...
    validateGenres: function ( e ) {
        ...
        if ( AlbumValidator.validateInputs( this.collectGenres(), 1, 'genres', $input ) ) {
            ...
            $input.find('.error-message').remove()
            // המשתנה $input מכיל את האלמנט div שיש לו מזהה ייחודי בשם album-genres מוצא בתוכו אלמנט div שיש לו מזהה ייחודי בשם tags ומסיר ממנו את ה- class בשם error ומוסיף לו את ה- class בשם success
