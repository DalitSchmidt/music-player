...
const AlbumForm = {
    ...
    collectSongs: function() {
        ...
        $.each( $('.song-item'), ( index, item ) => {
            ...
            if ( name !== '' && duration !== '' && id !== '' ) {
                ...
                // המשתנה isIdExists מפעיל את הפונקציה isInArrayOfObjects המצויה תחת האובייקט Utils ושבאמצעותה אנו בודקים אם המערך הוא מערך של אובייקטים ולצורך כך היא מקבלת את המשתנים songs, 'id' ו- id
                ...
                // הצגת המשתנה songs המכיל מערך של השירים בחלון ב- console
                console.log( songs )
            }
        })

        // הפעלת הפונקציה validateInputs המצויה תחת האובייקט Validator ושבאמצעותה אנו מבעים בדיקת ולידציה לנתונים ב- inputים שהם למעשה מערך כגון שירים, ז'אנרים וכו' ולצורך כך היא מקבלת את המשתנים songs, 5, 'songs_youtube_id' ואת האלמנט div שיש לו מזהה ייחודי בשם add-album-playlist-form
        Validator.validateInputs( songs, 5, 'songs_youtube_id', $('#add-album-playlist-form') )
        // הפונקציה מחזירה את אורך המשתנה songs המכיל מערך עם אובייקטים של השירים שבתוכו מצויים הפרופרטיס name, youtube ו- duration, ואם אין נתונים במשתנה songs הפונקציה מחזירה את הערך הבוליאני false
    ...
    saveAlbum: function( e ) {
        ...
        if ( !album ) {
            // הפעלה של הפונקציה scrollTop המבצעת גלילה של העמוד חזרה למעלה עד לתחילת האלמנט form שיש לו מזהה ייחודי בשם add-new-album
        ...
        if( !songs ) {
            // הפעלה של הפונקציה scrollTop המבצעת גלילה של העמוד חזרה למעלה עד לתחילת האלמנט div שיש לו מזהה ייחודי בשם add-album-playlist-title
