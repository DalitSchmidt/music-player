...
import $ from 'jquery'
// ייבוא היכולות של האובייקט DataService על-מנת שהאובייקט AlbumPlayer יוכל להשתמש בהן
...
import AlbumTemplates from '../Templates/SearchResultsTemplate'
...
const AlbumPlayer = {
    ...
    getAlbum: function () {
        ...
        // הפעלה של הפונקציה getAlbumById (המבצעת בקשת getJSON לנתיב 'http://localhost:3000/api/albums/' + album_id, ובכך מתאפשר למעשה לקבל אלבום ספציפי לפי המספר id שלו) ושמצויה תחת האובייקט בשם DataService אשר מקבלת את המשתנה id (המפעיל את הפונקציה getAlbumID שבאמצעותה מתאפשר לקבל את המספר id של האלבום המצוי ב- URL), ולאחר מכן נפעיל promise המקבל את המשתנה album ומפעיל פונקציות שונות הקשורות להצגה של האלבום ב- DOM
